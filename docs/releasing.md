...
3. Merge the reviewed version change to `main` after CI passes. Fetch current `main` and create/push its annotated version tag, for example:
   ...
   git switch main
   git pull --ff-only
   git tag -a v0.4.8 -m 'zStudio 0.4.8'
...
```powershell
./tools/verify-package.ps1 -Directory artifacts/zStudio-win-x64 -Archive artifacts/zStudio-0.4.8-win-x64.zip -ExpectedVersion 0.4.8
```
